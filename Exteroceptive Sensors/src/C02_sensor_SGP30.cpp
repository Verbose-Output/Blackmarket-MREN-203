...
void loop()
{
  if (millis() - measurement_time > period)
  {
    ...
    // humidity to enable the humidity compensation for the air quality signals
    ...
    // Optional: you can also get the raw measurements from the SGP30, if
    // you're interested
    //
    ...
    // Optional: you can check for and output the current baseline values
    // from the sensor
    
    counter++;
    if (counter == 30) // 30 is not calculated; just a "large enough" number of measurements
    {
      counter = 0;
